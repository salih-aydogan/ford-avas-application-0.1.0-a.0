 ...
 *
 *END**************************************************************************/
void FLEXIO_SPI_DRV_MasterEnableTransfer(flexio_spi_master_state_t *master)
{
    FLEXIO_Type *baseAddr;
    uint8_t resourceIndex;    /* Index of first used internal resource instance (shifter and timer) */
...
status_t FLEXIO_SPI_DRV_MasterTransmitBlocking(flexio_spi_master_state_t *master,
                                                      const uint8_t *txData)
{
    /* Initialize transfer data */
	uint32_t data = (uint32_t)(*(const uint16_t *)txData);

	/* Shift data before bit-swapping it to get the relevant bits in the lower part of the shifter */
	data <<= 32U - (8U * (uint32_t)(master->transferSize));

	FLEXIO_WriteShifterBuffer(g_flexioBase[master->flexioCommon.instance],
							  TX_SHIFTER(master->flexioCommon.resourceIndex),
							  data,
							  FLEXIO_SHIFTER_RW_MODE_BIT_SWAP);
    return STATUS_SUCCESS;
}

void FLEXIO_SPI_DRV_MasterTransmitFixedSizeBlocking(flexio_spi_master_state_t *master,
                                                      const uint8_t *txData)
{
    /* Initialize transfer data */
	uint32_t data = (uint32_t)(*(const uint16_t *)txData);

    if(master->bitOrder == FLEXIO_SPI_TRANSFER_MSB_FIRST)
    {
        /* Shift data before bit-swapping it to get the relevant bits in the lower part of the shifter */
        data <<= 32U - (8U * (uint32_t)(master->transferSize));
