...
static status_t POWER_DRV_EnableVlpClockSrc(void);

static status_t POWER_DRV_DisableVlpClockSrc(void);
    ...
    switch (configPtr->powerMode)
    {
        ...
        case POWER_MANAGER_RUN:
            ...
            if ((returnCode == STATUS_SUCCESS) && changeClkVlp)
            {
                ...
                returnCode = POWER_DRV_EnableVlpClockSrc();
                if (returnCode == STATUS_SUCCESS)
                {
            ...
            if (currentMode != STAT_VLPR)
            {
                ...
                if (STATUS_SUCCESS == returnCode)
                {
                    ...
                    if (STATUS_SUCCESS == returnCode)
                    {
                        ...
                        returnCode = POWER_DRV_DisableVlpClockSrc();                        
                    }
                    if (returnCode == STATUS_SUCCESS)
                    {
                        /* Switch the mode */
...
static status_t POWER_DRV_EnableVlpClockSrc(void)
{
    ...
    while ((POWER_SCG_GetFircStatus(SCG) != enableFIRC) && (timeOut > 0U));
    if (timeOut == 0U)
    {
        status = STATUS_ERROR;
    }
    ...
    while ((POWER_SCG_GetSoscStatus(SCG) != enableSOSC) && (timeOut > 0U));
    if (timeOut == 0U)
    {
        status = STATUS_ERROR;
    }
    ...
    while ((POWER_SCG_GetSpllStatus(SCG) != enableSPLL) && (timeOut > 0U));
    if (timeOut == 0U)
    {
        status = STATUS_ERROR;
    }
...
static status_t POWER_DRV_DisableVlpClockSrc(void)
{
    ...
    while ((POWER_SCG_GetSpllStatus(SCG) != false) && (timeOut > 0U));
    if (timeOut == 0U)
    {
        status = STATUS_ERROR;
    }
    ...
    while ((POWER_SCG_GetSoscStatus(SCG) != false) && (timeOut > 0U));
    if (timeOut == 0U)
    {
        status = STATUS_ERROR;
    }
    ...
    while ((POWER_SCG_GetFircStatus(SCG) != false) && (timeOut > 0U));
    if (timeOut == 0U)
    {
        status = STATUS_ERROR;
    }
